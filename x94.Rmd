...
plotClusters(cl2, whichClusters = c(1, 2), colData = c("gfp"), existingColors = "firstOnly")
```
...
genes <- getBestFeatures(cl2, contrastType = "Dendro", DEMethod="limma-voom", number = 100)
head(genes)
...
genes_1vall <- getBestFeatures(cl2, contrastType = "OneAgainstAll",
                               DEMethod="limma",
                               number=100)
...
genes_5 <- getBestFeatures(cl2, contrastType = "OneAgainstAll", DEMethod="limma",
                               number=5)
...
genes_5 <- getBestFeatures(cl2, contrastType = "Dendro", DEMethod="limma",
                               number=5)
...
top <- getBestFeatures(cl2, contrastType="Dendro", DEMethod="limma", number=Inf, p.value=0.05)

xx <- factor(primaryClusterNamed(cl2), 
             levels=paste0("S", 0:(nClusters(cl2)[1]-1)))
...
top <- getBestFeatures(cl2, contrastType = "OneAgainstAll", DEMethod="limma", number=Inf, p.value=0.05)
...
markers <- list(m1=c("Ddit4l", "Whrn", "Plb1", "Aldh1l1", "Endou", "Thsd7a",
                     "Spock3", "Car4"),
...
genes_5b <- getBestFeatures(cl2, contrastType = "OneAgainstAll",
                            DEMethod="limma-voom", number=5)
...
save(cl2, file="data/clusterexperiment_all_batches.rda")
```
...
```{r scmap, cache=FALSE, results="asis", eval=FALSE}
...
```{r scmap_table, dependson = "scmap", eval=FALSE}
